...
const createPassengerController = async (req, res) => {
  try {
    ...
    passengers.forEach((passenger) => {
      ...
      if (!flight) {
        throw new Error(
          `Flight with ID ${passenger.ticket.flight_id} not found`
        );
      }
    ...
    const createdPassengers = await Promise.all(
      passengers.map(async (passenger) => {
        ...
        const flightSeatsDoc = await FlightSeats.findOne({
          flightId: flight.id,
        });
        if (!flightSeatsDoc) {
        ...
        const seatIndex = flightSeatsDoc.seats.findIndex(
          (seat) => seat.seatNumber === passenger.seat_number
        );
        if (seatIndex === -1 || flightSeatsDoc.seats[seatIndex].isBooked) {
          throw new ErrorWithStatusCode(
            `Seat ${passenger.seat_number} is not available`,
            200
          );
        }
        ...
        flightSeatsDoc.seats[seatIndex] = seat;
    ...
    req.io.emit("notification-booking", notificationData);
